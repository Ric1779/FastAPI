...
async def login_for_access_token(
    ...
    # Don't reveal which one failed (security best practices)
...
async def forgot_password(
    ...
    if user:
        ...
        # Note: only simple data is passed, the database Session isn't passed 
        # to background task, background task runs after the response is sent
        # so the session maybe closed by then 
...
async def update_user(
    ...
    # Note: removed the below because we handle the profile pic update/delete through 
    # it's dedicated endpoints with authorization checks
    
    # if user_update.image_file is not None:
    #     user.image_file = user_update.image_file

    # Note: similar to post patch endpoint
